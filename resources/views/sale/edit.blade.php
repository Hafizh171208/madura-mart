...
<div class="card-body"><form action="{{ route('sale.update', $data->id)}}" method="POST">@csrf @method('PUT')
<div class="row">
<div class="col-md-6 mb-3"><label class="form-label">No Struk</label><input type="text" class="form-control" name="no_struk" value="{{$data->no_struk}}" required></div>
<div class="col-md-6 mb-3"><label class="form-label">Tanggal Jual</label><input type="date" class="form-control" name="tgl_jual" value="{{$data->tgl_jual}}" required></div>
...
<div class="text-end"><a href="{{ route('sale.index')}}" class="btn bg-gradient-secondary me-3">Cancel</a><button type="submit" class="btn bg-gradient-primary">Update</button></div>
